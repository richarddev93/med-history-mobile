...
export const peoplesApi = {
  async getAll() {
    try {
      const response = await http.get('/v1/person')
      ...
      if (error instanceof Error) console.error('Erro ao listar pessoas:', error.message)
  ...
  async getById(id: string) {
    try {
      ...
      if (error instanceof Error) console.error('Erro ao buscar pessoa:', error.message)
  ...
  async create(data: CreatePersonData) {
    try {
      ...
      const response = await http.post('v1/person', body)
      const parsed = PersonSchema.safeParse(response.data)
      ...
      if (error instanceof Error) console.error('Erro ao criar pessoa:', error.message)
